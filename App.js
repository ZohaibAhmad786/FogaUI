...
import { View, Text, StatusBar,able, Image, ActivityIndicator, SafeAreaView, Alert, StyleSheet, TouchableOpacity, RecyclerViewBackedScrollView } from "react-native";
...
const App = () => { 
  ...
  const INJECTEDJAVASCRIPT ="setTimeout(() => {document.addEventListener('scroll', function (event) {window.ReactNativeWebView.postMessage(JSON.stringify(document.getElementsByClassName('topconteiner')[0].scrollTop));},true);}, 300);true;"             
  ...
  return (
    <SafeAreaView style={{ flex: 1, justifyContent: "center", marginTop: StatusBar.currentHeight , backgroundColor: 'red'}}>
      <WebView
        source={{ uri: "https://foga.app" }}
        onNavigationStateChange={(navState) => {setLink(navState.url)}}
         ref={webviewRef }
